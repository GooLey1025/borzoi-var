...
class Job:
    ...
    def __init__(self, cmd, name, out_file=None, err_file=None, sb_file=None,
        ...
        self.queue = queue # Your gpu partition name in SLURM. My here is 'rtx4070' or 'gpu1'
        # self.queue = self.translate_gpu(queue) # I think this is not necessary
        self.cpu = cpu
        self.mem = mem
        self.time = time
        self.gpu = gpu
    ...
    def launch(self):
        ...
        if self.gpu > 0:
            if self.queue == "" or self.queue == 'gpu':
                gpu_str = 'gpu'
                gres_str = '--gres=gpu'
            elif self.queue in ['nvidia_geforce_rtx_4090']:
                if self.queue == 'nvidia_geforce_rtx_4090':
                    gpu_str = 'minigpu'
                gres_str = f'--gres=gpu:{self.queue}'
            elif self.queue in ['rtx4070']: # In case of my local server, which include rtx4070
                gpu_str = 'rtx4070'
                gres_str = f'--gres=gpu:{self.queue}'
            elif self.queue in ['gpu1']: # In case of HPC server of school, which include A100
                gpu_str = 'gpu1'
                gres_str = f'--gres=gpu'
            elif self.queue in ['gpu2']: # In case of HPC server of school, which include A100
                gpu_str = 'gpu2'
                gres_str = f'--gres=gpu'
            else:
                gpu_str = 'gpu'
    ...
    def translate_gpu(self, queue_gpu):
        ...
        translation = {
            ...
            'rtx4070': 'rtx4070',  # In case of my environment
            'a100-gpu1': 'gpu1', # in case of HPC server of school
            'a100-gpu2': 'gpu2'
        }
